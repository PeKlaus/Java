...
public class Zweiampelsteuerung extends JFrame {
	...
	contentPane.setLayout(new MigLayout("", "[7.00,grow][grow][grow]",
		"[grow]"));
    //            >>>> AMPEL1 <<<<
	// übergibt im Konstruktor die Zeiten der einzelnen Phasen
	...
	btnStart.addActionListener(new ActionListener() {
		...
		Thread t1 = new Thread(panelAmpel1); // erzeugt den Thread für  panelAmpel1
		panelAmpel1.automatik = true;        // prüft auf automatik = true
		t1.start();                          // Startet den Thread 
		Thread t2 = new Thread(panelAmpel2); // erzeugt den Thread für  panelAmpel2
		panelAmpel2.automatik = true;        // prüft auf automatik = true
		t2.start();                          // Startet den Thread
	    }
	});
	...
	btnStopp.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
	  // Beide Ampeln auf automatik = false setzen  	
